...
def draw_boxes(img, bbox, identities=None, offset=(0,0), with_bboxWH=True):    
    for i,box in enumerate(bbox):
        ...
        if with_bboxWH:
            cv2.rectangle(img,(x1, y1),(x1+x2,y1+y2),color,3) #KIP: modified, before was 
...
def parse_args():
    ...
    parser.add_argument("--detwith_bboxWH", type=bool, default=True) #KIP: detection is with bboxes annotated with top-left coord and bbox width and height
